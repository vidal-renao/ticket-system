...
Phase 4A must run first on a disposable Preview test database following `docs/runbooks/rag-schema-migration.md`. Independent re-review precedes execution. Rollback initially disables all v2 consumers; because v2 has no production consumer or backfill, objects may be removed only in Preview after evidence export and dependency inspection. Production direct application is prohibited.
