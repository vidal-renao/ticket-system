...
Phase 4A.8 makes embedded chunks immutable: changing content, hash, model, dimension, tenant, document, position or version requires a new document version/chunk generation. The only permitted ready transition is ready to stale with the vector cleared and semantic fields unchanged. Approval revocation performs that idempotent invalidation in the version transaction without taking a reverse version lock from the chunk trigger. This is schema enforcement, not evidence that sanitization exists.
