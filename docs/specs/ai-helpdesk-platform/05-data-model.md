...
Corrections narrow this contract: deleted source/document/version state is terminal; ready requires processing; a ready chunk cannot be edited except for controlled vector-clearing invalidation; stale content requires a new generation. Embedding jobs use `completed` for successful terminal attempts, retain completed/failed history with `attempt_number`, and require each retry to reference the immediately preceding failed/stale attempt in the same tenant/version. A partial unique index permits only one pending/processing job per version.
