...
Phase 4A adds unit/static coverage and a migration-driven opt-in disposable-database harness. Local mode requires loopback and a guarded database suffix; Preview mode resolves official branch metadata before database access, proves the target is a healthy disposable child and matches that child ref to the connection. The runner applies the real migrations, executes 82 assertions across Alpha/Beta roles, authenticated/backend RPCs, lifecycle exclusion, RLS, archival, supersession, grants, constraints, ready-content immutability, approval invalidation and multi-attempt job history, then proves re-execution fails fast. It repeats a real two-connection race three times, launching the second session only after the first emits a unique lock-acquired barrier, and fails on deadlock, lock/statement timeout, process timeout or an invalid committed state. Static/mocked tests remain supporting evidence; PostgreSQL execution is still pending.
