...
export function statusColor(status: string): string {
  const map: Record<string, string> = {
    ...
    resolved: "text-violet-300 bg-violet-500/10 border-violet-500/25",
...
export function availabilityStatusColor(status: string | null | undefined): string {
  const map: Record<string, string> = {
    online: "bg-emerald-400",
    busy: "bg-orange-400",
    offline: "bg-slate-500",
  };
  return map[status ?? "offline"] ?? "bg-slate-500";
}
