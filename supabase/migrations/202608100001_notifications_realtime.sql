...
-- the `supabase_realtime` publication. `public.hd_notifications` was never added,
...
--     evaluates per subscriber. `hd_notifications` has rowsecurity enabled and
...
--   ALTER PUBLICATION supabase_realtime DROP TABLE public.hd_notifications;
-- ============================================================================
...
begin
  ...
  if not exists (
    ...
    where pubname = 'supabase_realtime'
      ...
      and tablename = 'hd_notifications'
  ) then
    alter publication supabase_realtime add table public.hd_notifications;
  end if;
end
$$;
